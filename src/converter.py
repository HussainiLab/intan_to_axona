...
from src.csv_to_pos import write_pos, pd_to_dict, text_to_dict
...
def convert_files(target_directory, rhd_file_path, txt_or_csv_path=None, json_pos_path=None):
    ...
    if txt_or_csv_path is not None and json_pos_path is not None:
        if txt_or_csv_path.endswith('.csv'):
            df = pd.read_csv(txt_or_csv_path)
            pos_dict = pd_to_dict(df)
        elif txt_or_csv_path.endswith('.txt'):
